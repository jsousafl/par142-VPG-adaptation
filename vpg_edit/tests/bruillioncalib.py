...
from urx import Robot as roboturx
...
# --------------- Setup options ---------------
"""
tcp_host_ip = '192.186.1.5' # IP and port to robot arm as TCP client (UR5)
tcp_port = 30002
rtc_host_ip = '100.127.7.223' # IP and port to robot arm as real-time client (UR5)
rtc_port = 30003
"""

ip_config = '192.186.1.5'
...
"""robot = Robot(False, None, None, workspace_limits,
              ...
              False, None, None)"""
robot = urx.roboturx(ip_config)
#WE HAVE TO CHANGE TO HOME POSITION
...
robot.movej([-np.pi, -np.pi/2, np.pi/2, 0, np.pi/2, np.pi])

# Move robot to each calibration point in workspace
print('Collecting data...')
for calib_pt_idx in range(num_calib_grid_pts):
    tool_position = calib_grid_pts[calib_pt_idx,:]
    robot.movel(tool_position, tool_orientation,tool_acc,tool_vel)
    #time.sleep(1)
