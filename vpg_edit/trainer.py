...
from utilsedit import CrossEntropyLoss2d
...
class Trainer(object):
    ...
    def forward(self, color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=-1):
        ...
        diag_length = np.sqrt(np.power(float(color_heightmap_2x.shape[0]),2)+np.power(float(color_heightmap_2x.shape[1]),2))
        ...
        if self.method == 'reactive':
            ...
            for rotate_idx in range(len(output_prob)):
                if rotate_idx == 0:
                    push_predictions = output_prob[rotate_idx][0].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[1]/2 - padding_width/2)]
                    grasp_predictions = output_prob[rotate_idx][1].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[1]/2 - padding_width/2)]
                else:
                    push_predictions = np.concatenate((push_predictions, output_prob[rotate_idx][0].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[1]/2 - padding_width/2)]), axis=0)
                    grasp_predictions = np.concatenate((grasp_predictions, output_prob[rotate_idx][1].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[1]/2 - padding_width/2)]), axis=0)
    ...
    def backprop(self, color_heightmap, depth_heightmap, primitive_action, best_pix_ind, label_value):
        ...
        if self.method == 'reactive':
            ...
            # Compute labels
            label = np.zeros((1,color_heightmap.shape[0],color_heightmap.shape[1]))
            diag_length = np.sqrt(np.power(float(color_heightmap.shape[0]),2)+np.power(float(color_heightmap.shape[1]),2))
            diag_length = np.ceil(diag_length/32)*32
            padding_width = int((diag_length - color_heightmap.shape[0])/2)
            action_area = np.zeros((color_heightmap.shape[0]-2*padding_width,color_heightmap.shape[1]-2*padding_width))
            action_area[best_pix_ind[1]][best_pix_ind[2]] = 1
            # blur_kernel = np.ones((5,5),np.float32)/25
            # action_area = cv2.filter2D(action_area, -1, blur_kernel)
            tmp_label = np.zeros((color_heightmap.shape[0]-2*padding_width,color_heightmap.shape[1]-2*padding_width))
            tmp_label[action_area > 0] = label_value
            label[0,padding_width/2:(color_heightmap.shape[0]-padding_width/2),padding_width/2:(color_heightmap.shape[1]-padding_width/2)] = tmp_label

            # Compute label mask
            label_weights = np.zeros(label.shape)
            tmp_label_weights = np.zeros((color_heightmap.shape[0]-2*padding_width,color_heightmap.shape[1]-2*padding_width))
            tmp_label_weights[action_area > 0] = 1
            label_weights[0,padding_width/2:(color_heightmap.shape[0]-padding_width/2),padding_width/2:(color_heightmap.shape[1]-padding_width/2)] = tmp_label_weights
            ...
            if primitive_action == 'push':
                ...
                if self.use_cuda:
                    loss = self.criterion(self.model.output_prob[0][0].view(1,color_heightmap.shape[0],color_heightmap.shape[1]), Variable(torch.from_numpy(label).float().cuda())) * Variable(torch.from_numpy(label_weights).float().cuda(),requires_grad=False)
                else:
                    loss = self.criterion(self.model.output_prob[0][0].view(1,color_heightmap.shape[0],color_heightmap.shape[1]), Variable(torch.from_numpy(label).float())) * Variable(torch.from_numpy(label_weights).float(),requires_grad=False)
                loss = loss.sum()
                loss.backward()
                loss_value = loss.cpu().data.numpy()[0]
                ...
                if self.use_cuda:
                    loss = self.criterion(self.model.output_prob[0][1].view(1,color_heightmap.shape[0],color_heightmap.shape[1]), Variable(torch.from_numpy(label).float().cuda())) * Variable(torch.from_numpy(label_weights).float().cuda(),requires_grad=False)
                else:
                    loss = self.criterion(self.model.output_prob[0][1].view(1,color_heightmap.shape[0],color_heightmap.shape[1]), Variable(torch.from_numpy(label).float())) * Variable(torch.from_numpy(label_weights).float(),requires_grad=False)
                loss = loss.sum()
                loss.backward()
                loss_value = loss.cpu().data.numpy()[0]
