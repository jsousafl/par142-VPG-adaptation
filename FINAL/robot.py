...
# ADDITION OF THE URX LIBRARY FOR READING DATA AND SEND COMMANDS
# ERASE 
...
class Robot(object):
    def __init__(self, is_sim, obj_mesh_dir, num_obj, workspace_limits,
        ...
        if self.is_sim:
            ...
            self.sim_client = vrep.simxStart('192.168.1.3', 19997, True, True, 5000, 5) # Connect to V-REP on port 19997
        ...
        # If in real-settings...
            ...
            self.home_joint_config = [(50.7/360.0)*2*np.pi, (-98.0/360.0)*2*np.pi, (-85.0/360.0)*2*np.pi, (-62.0/360.0)*2*np.pi, (86.0/360.0)*2*np.pi, 0.0]
    ...
    def setup_sim_camera(self): #simulation method
        ...
        cam_rotm = np.eye(4,4)
        cam_rotm[0:3,0:3] = np.linalg.inv(utilsedit.euler2rotm(cam_orientation))
    ...
    def add_objects(self):
        ...
        for object_idx in range(len(self.obj_mesh_ind)):
            ...
            #real_start_name = "/home/ecl/robot/visual-pushing-grasping/objects/blocks"
            real_start_name = "/home/jsousafl/Documents/PAr_Robotique_en_Vrac/code_original/visual-pushing-grasping/objects/blocks"
            final_name = curr_mesh_file[-6:len(curr_mesh_file)]
            curr_mesh_file = real_start_name + final_name
            print(curr_mesh_file)
            #print(curr_mesh_file[-6:len(curr_mesh_file)])
    ...
    def restart_sim(self): #simulation method
    ...
    def check_sim(self): #simulation method
    ...
    def get_task_score(self): #simulation method
    ...
    def check_goal_reached(self): #simulation method
    ...
    def get_obj_positions(self): #simulation method
    ...
    def get_obj_positions_and_orientations(self): #simulation method
    ...
    def reposition_objects(self, workspace_limits): #simulation method
    ...
    def get_camera_data(self): #hybrid method : for unique calling
    ...
    def get_urx_data(self,subpackage):
        if subpackage == 'cartesian_info':
            false_rob = urx.Robot(self.tcp_host_ip)
            actual_tool_pose = false_rob.getl(wait=True)
            false_rob.close()
            return actual_tool_pose
        elif subpackage == 'joint_data': #ERASABLE
            false_rob = urx.Robot(self.tcp_host_ip)
            actual_joint_positions = false_rob.getj(wait=True)
            false_rob.close()
            return actual_joint_positions
        else:
            print("Error: subpackage not valid")
            return 0
    ...
    def close_gripper(self, async=False):
        ...
        if self.is_sim:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                s.send(b'GET POS')
                data = s.recv(1024)
                #while (not gripper_fully_closed) and (int.from_bytes(data,byteorder='big') < 6382444):
                while (not gripper_fully_closed) and (int(repr(data)[2:-1]) < 223):
                    s.send(b'GET POS')
                    data = s.recv(1024)
                    gripper_fully_closed = self.check_grasp()
                s.close()
    ...
    def open_gripper(self, async=False):
        ...
        if self.is_sim:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                s.send(b'GET POS')
                data = s.recv(1024)
               #while int.from_bytes(data,byteorder='big') > 58:
                while int(repr(data)[2:-1]) > 95:
                    s.send(b'GET POS')
                    data = s.recv(1024)
                s.close()
    ...
    def move_to(self, tool_position, tool_orientation):
        ...
        if self.is_sim:
            ...
            # sim_ret, UR3_target_handle = vrep.simxGetObjectHandle(self.sim_client,'UR3_target',vrep.simx_opmode_blocking)
            sim_ret, UR3_target_position = vrep.simxGetObjectPosition(self.sim_client, self.UR3_target_handle,-1,vrep.simx_opmode_blocking)

            move_direction = np.asarray([tool_position[0] - UR3_target_position[0], tool_position[1] - UR3_target_position[1], tool_position[2] - UR3_target_position[2]])
            move_magnitude = np.linalg.norm(move_direction)
            move_step = 0.02*move_direction/move_magnitude
            num_move_steps = int(np.floor(move_magnitude/0.02))

            for step_iter in range(num_move_steps):
                vrep.simxSetObjectPosition(self.sim_client,self.UR3_target_handle,-1,(UR3_target_position[0] + move_step[0], UR3_target_position[1] + move_step[1], UR3_target_position[2] + move_step[2]),vrep.simx_opmode_blocking)
                sim_ret, UR3_target_position = vrep.simxGetObjectPosition(self.sim_client,self.UR3_target_handle,-1,vrep.simx_opmode_blocking)
            vrep.simxSetObjectPosition(self.sim_client,self.UR3_target_handle,-1,(tool_position[0],tool_position[1],tool_position[2]),vrep.simx_opmode_blocking)
            ...
            pose = np.concatenate((tool_position,tool_orientation))
            rob = urx.Robot(self.tcp_host_ip)
            rob.movex('movej',pose,self.tool_acc,self.tool_vel)
            rob.close()
            #check acc and vel
            """# Block until robot reaches target tool position
            
            #tcp_state_data = self.tcp_socket.recv(2048)
            actual_tool_pose = self.get_urx_data('cartesian_info')
    ...
    def move_joints(self, joint_configuration):
        ...
        self.tcp_socket.send(str.encode(tcp_command))"""
        rob = urx.Robot(self.tcp_host_ip)
        rob.movej(joint_configuration,self.joint_acc,self.joint_vel)
        rob.close()
    ...
    def check_grasp(self):
        ...
        if int.from_bytes(data,byteorder='little') == 49:
...
#        return tool_analog_input2 > 0.26
    ...
    def grasp(self, position, heightmap_rotation_angle, workspace_limits):
        ...
        if self.is_sim:
            ...
            sim_ret, gripper_orientation = vrep.simxGetObjectOrientation(self.sim_client, self.UR3_target_handle, -1, vrep.simx_opmode_blocking)
            rotation_step = 0.3 if (tool_rotation_angle - gripper_orientation[1] > 0) else -0.3
            num_rotation_steps = int(np.floor((tool_rotation_angle - gripper_orientation[1])/rotation_step))

            # Simultaneously move and rotate gripper
            for step_iter in range(max(num_move_steps, num_rotation_steps)):
                vrep.simxSetObjectPosition(self.sim_client,self.UR3_target_handle,-1,(UR3_target_position[0] + move_step[0]*min(step_iter,num_move_steps), UR3_target_position[1] + move_step[1]*min(step_iter,num_move_steps), UR3_target_position[2] + move_step[2]*min(step_iter,num_move_steps)),vrep.simx_opmode_blocking)
                vrep.simxSetObjectOrientation(self.sim_client, self.UR3_target_handle, -1, (np.pi/2, gripper_orientation[1] + rotation_step*min(step_iter,num_rotation_steps), np.pi/2), vrep.simx_opmode_blocking)
            vrep.simxSetObjectPosition(self.sim_client,self.UR3_target_handle,-1,(tool_position[0],tool_position[1],tool_position[2]),vrep.simx_opmode_blocking)
            vrep.simxSetObjectOrientation(self.sim_client, self.UR3_target_handle, -1, (np.pi/2, tool_rotation_angle, np.pi/2), vrep.simx_opmode_blocking)
            ...
            print('---------------------------------- GRASP MOUVEMENT ----------------------------------')
            home_position = [-0.03986280939897746, 0.3393151238887547, 0.1]
            bin_position = [0.22593101998862428, -0.09103914827098063, 0.2]
            SECURITY_POSITION = [0.21179907520422353, 0.2244808105593349, 0.12]

            
            # Compute tool orientation from heightmap rotation angle
            grasp_orientation = [-1.0,0.0]
            heightmap_rotation_angle = - heightmap_rotation_angle - np.deg2rad(41.8)
...
#            tool_orientation_rotm = utilsedit.angle2rotm(tool_orientation_angle, tool_orientation_axis, point=None)[:3,:3]
#            
#            # Compute tilted tool orientation during dropping into bin
#            tilt_rotm = utilsedit.euler2rotm(np.asarray([-np.pi/4,0,0]))
#            tilted_tool_orientation_rotm = np.dot(tilt_rotm, tool_orientation_rotm)
#            tilted_tool_orientation_axis_angle = utilsedit.rotm2angle(tilted_tool_orientation_rotm)
...
#            tilted_tool_orientation = tool_orientation
            ...
            # Grqsp only within workspace limits
            position = np.asarray(position).copy()
            position_altern = utilsedit.cart2altern(np.transpose(position))
            position_altern[0] = min(max(position_altern[0], workspace_limits[0][0]), workspace_limits[0][1])
            position_altern[1] = min(max(position_altern[1], workspace_limits[1][0]), workspace_limits[1][1])
            #position_altern[2] = min(max(position_altern[2]-0.02, workspace_limits[2][0]),workspace_limits[2][1]) # Add buffer to surface
            position_altern[2] = workspace_limits[2][0]
            position = utilsedit.altern2cart(position_altern)
            ...
            if gripper_open:
                ...
                self.move_to([position[0],position[1],position[2]+0.08], [tool_orientation[0],tool_orientation[1],tool_orientation[2]]);
#                self.move_to([bin_position[0],bin_position[1],bin_position[2]], [tool_orientation[0],tool_orientation[1],tool_orientation[2]]);
                self.open_gripper();
                self.move_to([SECURITY_POSITION[0], SECURITY_POSITION[1], SECURITY_POSITION[2]], [tool_orientation[0],tool_orientation[1],tool_orientation[2]]);
                self.move_to([home_position[0],home_position[1],home_position[2]], [tool_orientation[0],tool_orientation[1],tool_orientation[2]]);
                if self.check_grasp():
                    grasp_success = True
...
#            tool_orientation_rotm = utilsedit.angle2rotm(tool_orientation_angle, tool_orientation_axis, point=None)[:3,:3]
            ...
            push_length = 0.1
            print(np.rad2deg(heightmap_rotation_angle))
            heightmap_rotation_angle = - heightmap_rotation_angle - np.deg2rad(41.8)
...
#            push_direction = -push_direction
            print("_________________________________________PUSH DIRECTION_______________")
            print(push_direction)
            print(np.rad2deg(heightmap_rotation_angle))
            target_x = position[0] + push_direction[0]*push_length
            target_y = position[1] + push_direction[1]*push_length
            target_z = position[2]
            target_u_v_z = utilsedit.cart2altern(np.transpose([target_x,target_y,target_z]))
            target_u = min(max(target_u_v_z[0], workspace_limits[0][0]), workspace_limits[0][1])
            target_v = min(max(target_u_v_z[1], workspace_limits[1][0]), workspace_limits[1][1])
            target_u_v_z = np.transpose(np.array([target_u, target_v, target_z]))
            target_pos_cart = utilsedit.altern2cart(target_u_v_z)
            #push_endpoint = np.asarray([target_x, target_y, position[2]])
            push_endpoint = target_pos_cart
#            push_direction.shape = (3,1)

#            # Compute tilted tool orientation during push
#            tilt_axis = np.dot(utilsedit.euler2rotm(np.asarray([0,0,np.pi/2]))[:3,:3], push_direction)
#            tilt_rotm = utilsedit.angle2rotm(-np.pi/8, tilt_axis, point=None)[:3,:3]
#            tilted_tool_orientation_rotm = np.dot(tilt_rotm, tool_orientation_rotm)
#            tilted_tool_orientation_axis_angle = utilsedit.rotm2angle(tilted_tool_orientation_rotm)
#            tilted_tool_orientation = tilted_tool_orientation_axis_angle[0]*np.asarray(tilted_tool_orientation_axis_angle[1:4])
#
#            tilted_tool_orientation = tool_orientation
            ...
            position_altern[0] = min(max(position_altern[0], workspace_limits[0][0]), workspace_limits[0][1])
            position_altern[1] = min(max(position_altern[1], workspace_limits[1][0]), workspace_limits[1][1])
            #position_altern[2] = max(position_altern[2]+0.01, workspace_limits[2][0]) # Add buffer to surface
            position = utilsedit.altern2cart(position_altern)
            ...
            #self.close_gripper()
            self.close_gripper();
            self.move_to([position[0],position[1],min(position[2]+0.1,workspace_limits[2][1])], [tool_orientation[0],tool_orientation[1],tool_orientation[2]])
            self.move_to([position[0],position[1],position[2]],[tool_orientation[0],tool_orientation[1],tool_orientation[2]])
            self.move_to([push_endpoint[0],push_endpoint[1],push_endpoint[2]],[tool_orientation[0],tool_orientation[1],tool_orientation[2]])
            self.move_to([SECURITY_POSITION[0],SECURITY_POSITION[1],SECURITY_POSITION[2]],[tool_orientation[0],tool_orientation[1],tool_orientation[2]])
            self.move_to([home_position[0],home_position[1],home_position[2]],[tool_orientation[0],tool_orientation[1],tool_orientation[2]])
    ...
    def restart_real(self):
        ...
        print('---------------------------------- RESTART MOUVEMENT ----------------------------------')
        grasp_orientation = [1.0,0.0]
        tool_rotation_angle = -np.pi/4
        tool_orientation = np.asarray([grasp_orientation[0]*np.cos(tool_rotation_angle) - grasp_orientation[1]*np.sin(tool_rotation_angle), grasp_orientation[0]*np.sin(tool_rotation_angle) + grasp_orientation[1]*np.cos(tool_rotation_angle), 0.0])*np.pi

        # Move to box grabbing position
        box_grab_position = [0.24944660697517906, -0.08208765216760248, 0.050]
#        self.move_to([box_grab_position[0],box_grab_position[1],box_grab_position[2]+0.12], [tool_orientation[0],tool_orientation[1],tool_orientation[2]])
#        self.open_gripper()
#        self.move_to([box_grab_position[0],box_grab_position[1],box_grab_position[2]],[tool_orientation[0],tool_orientation[1],tool_orientation[2]])
#        self.close_gripper()
        
        # Move to box release joint configuration 
        release_position = [0.128,0.313,0.050]
        home_position = [-0.03986280939897746, 0.3393151238887547, 0.1]
        SECURITY_POSITION = [0.21179907520422353, 0.2244808105593349, 0.12]
        tool_orientation_aux = [-2.90935492637218, 1.1854237052066265, 0.0]
        
#        self.move_to([release_position[0],release_position[1],release_position[2]], [tool_orientation_aux[0],tool_orientation_aux[1],tool_orientation_aux[2]])
#        self.move_to([release_position[0],release_position[1],release_position[2]+0.12], [tool_orientation_aux[0],tool_orientation_aux[1],tool_orientation_aux[2]])
#
#        self.move_to([box_grab_position[0],box_grab_position[1],box_grab_position[2]+0.12], [tool_orientation[0],tool_orientation[1],tool_orientation[2]])
#        self.move_to([box_grab_position[0],box_grab_position[1],box_grab_position[2]],[tool_orientation[0],tool_orientation[1],tool_orientation[2]])
#        self.open_gripper()
#        self.move_to([box_grab_position[0],box_grab_position[1],box_grab_position[2]+0.12], [tool_orientation[0],tool_orientation[1],tool_orientation[2]])
    ...
    def graph_move_to(self, tool_position, tool_orientation,valid_dh=None):
        #new scope of move_to
        rob = urx.Robot(self.tcp_host_ip)
        actual_tool_pose = rob.getl(wait=True)
        rob.close()
#        print(actual_tool_pose[0:3])
        actual_position_xyz = actual_tool_pose[0:3]
        actual_position_uvz = utilsedit.cart2altern(np.transpose(actual_position_xyz))
        tool_position_uvz = utilsedit.cart2altern(np.transpose(tool_position))
        
        ############# Lecture du fichier matlab #########################
        import scipy.io as sio
        from graphe import Graphe
        
         ############# Lecture du fichier matlab : FICHIER TEST ######################### 
        matlab_data = sio.loadmat('grid_robot.mat')
        matrix_adj = matlab_data['ADJ']
        pas = matlab_data['step'][0];
        matrix_nodes = matlab_data['M'] - 1
        nb_pts = matlab_data['N'][0] + 2
        
        ######### Initializing workspace and forbidden zone coordinates ###############################
        u_limits = np.array([-0.16, 0.16])     #CHANGE HERE = CHANGE MATLAB
        v_limits = np.array([0.0,0.32])        #CHANGE HERE = CHANGE MATLAB
        z_limits = np.array([0.0,0.32])       #CHANGE HERE = CHANGE MATLAB
        #u_forbidden = np.array([-0.1, 0,1])  #CHANGE HERE = CHANGE MATLAB
        #v_forbidden = np.array([0, 0.16])      #CHANGE HERE = CHANGE MATLAB
        u_forbidden = np.array([-0.1, 0.1])  #CHANGE HERE = CHANGE MATLAB
        v_forbidden = np.array([0.0, 0.14])      #CHANGE HERE = CHANGE MATLAB
        z_forbidden = np.array([0.16, 0.32])         #workspace_limits[2][1]
        
        
        cube_graph = Graphe(matlab_data,matrix_adj,pas,matrix_nodes,nb_pts,u_limits,v_limits,z_limits,u_forbidden,v_forbidden,z_forbidden)
        cube_graph.defining_obstacles(valid_dh)
#        if valid_dh!=None:
#            #Avoid obstacles
#            cube_graph.defining_obstacles(valid_dh)
        
        actual_pos_node = cube_graph.associate_node2coord(actual_position_uvz)
        tool_pos_node = cube_graph.associate_node2coord(tool_position_uvz)
        node_list_path = cube_graph.finding_minimal_path(actual_pos_node,tool_pos_node)
        matrix_coord_uvz_path = cube_graph.associate_coord2nodelist(node_list_path)
        for i in range(matrix_coord_uvz_path.shape[0]):
            target_position = utilsedit.altern2cart(np.transpose(matrix_coord_uvz_path[i]))
            self.move_to(target_position,tool_orientation)
    ...
    #         sim_ret,gripper_orientation = vrep.simxGetObjectOrientation(self.sim_client, UR3_target_handle, -1, vrep.simx_opmode_blocking)
    #         if tool_rotation_angle - gripper_orientation[1] > 0:
    #             increment = 0.2
    #         else:
    #             increment = -0.2
    #         while abs(tool_rotation_angle - gripper_orientation[1]) >= 0.2:
    #             vrep.simxSetObjectOrientation(self.sim_client, UR3_target_handle, -1, (np.pi/2, gripper_orientation[1] + increment, np.pi/2), vrep.simx_opmode_blocking)
    #             time.sleep(0.01)
    #             sim_ret,gripper_orientation = vrep.simxGetObjectOrientation(self.sim_client, UR3_target_handle, -1, vrep.simx_opmode_blocking)
    #         vrep.simxSetObjectOrientation(self.sim_client, UR3_target_handle, -1, (np.pi/2, tool_rotation_angle, np.pi/2), vrep.simx_opmode_blocking)
