...
def get_heightmap(color_img, depth_img, cam_intrinsics, cam_pose, workspace_limits, heightmap_resolution):
    ...
    # Redefine workspace_limits to create heigh0tmap of 224x224 pixels
    workspace_limits = [[-0.196,0.196],[0.08,0.472],[-0.005,0.12]]
    
    # Compute heightmap size
    heightmap_size = np.round(((abs(workspace_limits[1][1] - workspace_limits[1][0]))/heightmap_resolution, (abs(workspace_limits[0][1] - workspace_limits[0][0]))/heightmap_resolution)).astype(int)
    ...
    # Generating surface points in u v z coordinates
    
    #ATTENTION
#    print(np.prod(surface_pts.shape))
#    print("surface 0")
#    print(np.prod((surface_pts[:,0]).shape))
#    print("surface 1")
#    print(np.prod((surface_pts[:,1]).shape))
#    print("surface 2")
#    print(np.prod((surface_pts[:,2]).shape))
    
    #Rotation of pointcloud
    theta = 41.8*np.pi/180
    x_virtual = np.copy(surface_pts[:,0])
    y_virtual = np.copy(surface_pts[:,1])
    surface_pts[:,0] = x_virtual*math.cos(theta) - y_virtual*math.sin(theta)
    surface_pts[:,1] = x_virtual*math.sin(theta) + y_virtual*math.cos(theta)
    ...
    heightmap_pix_u = np.floor((surface_pts[:,0] - workspace_limits[0][0])/heightmap_resolution).astype(int)
    heightmap_pix_v = np.floor((surface_pts[:,1] - workspace_limits[1][0])/heightmap_resolution).astype(int)
    color_heightmap_r[heightmap_pix_v,heightmap_pix_u] = color_pts[:,[0]]
    color_heightmap_g[heightmap_pix_v,heightmap_pix_u] = color_pts[:,[1]]
    color_heightmap_b[heightmap_pix_v,heightmap_pix_u] = color_pts[:,[2]]
    color_heightmap = np.concatenate((color_heightmap_r, color_heightmap_g, color_heightmap_b), axis=2)
    depth_heightmap[heightmap_pix_v,heightmap_pix_u] = surface_pts[:,2]
    z_bottom = workspace_limits[2][0]
    depth_heightmap = depth_heightmap - z_bottom
    depth_heightmap[depth_heightmap < 0] = 0
    depth_heightmap[depth_heightmap == -z_bottom] = np.nan
#    color_heightmap = np.flipud(color_heightmap)
