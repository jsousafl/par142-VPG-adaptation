...
def main(args):
    ...
    def process_actions():
        while True:
            if nonlocal_variables['executing_action']:
                ...
                primitive_position_u_v = [-(best_pix_x-48) * heightmap_resolution + workspace_limits[0][1], (best_pix_y-30) * heightmap_resolution + workspace_limits[1][0], valid_depth_heightmap[best_pix_y][best_pix_x] + workspace_limits[2][0]]
                primitive_position = utilsedit.altern2cart(primitive_position_u_v)
    ...
    while True:
        ...
        #np.savetxt('testout',valid_depth_heightmap)

        # Reset simulation or pause real-world training if table is empty
        stuff_count = np.zeros(valid_depth_heightmap.shape)
        stuff_count[valid_depth_heightmap > 0.005] = 1
        stuff_count = stuff_count[0:170][47:175]
        logger.save_heightmaps(trainer.iteration, color_heightmap, stuff_count, '0')
        empty_threshold = 100
...
if __name__ == '__main__':
    ...
    parser.add_argument('--heightmap_resolution', dest='heightmap_resolution', type=float, action='store', default=0.00175, help='meters per pixel of heightmap')
