...
rtc_port = 30003
workspace_limits = np.asarray([[-0.112, 0.112], [0.16, 0.384], [0.0, 0.045]]) # Cols: min max, Rows: x y z (define workspace limits in robot coordinates)
checkerboard_offset_from_tool = [-0.04696,0.042,-0.04]
tool_orientation = [-2.8423740026858066, -1.2878981887531054, -0.047896657364744154] #DEFINED AND MEASURED ORIENTATION
point_origin_cube = utilsedit.altern2cart(np.transpose([workspace_limits[0][0],workspace_limits[1][0], 0.0])) # DEFINED AND MESURED ORIGIN POINT (FACE 1 AND 2)
point_end_face2 = utilsedit.altern2cart(np.transpose([workspace_limits[0][1],workspace_limits[1][0], 0.0])) # DEFINED AND MESURED END POINT OF FACE 2
point_end_face1 = utilsedit.altern2cart(np.transpose([workspace_limits[0][0],workspace_limits[1][1], 0.0])) # DEFINED AND MESURED END POINT OF FACE 1
...
SECURITY_ORIENTATION = [-2.8095352019503803, -1.143738023156609, 0.0]
